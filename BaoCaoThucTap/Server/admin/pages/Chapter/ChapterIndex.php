...
<?php
$countAllSql = "SELECT * FROM tbl_chapter";
...
if (isset($_GET['search'])) {
    $getTableDataSql = "SELECT * FROM tbl_chapter
    WHERE
        tbl_chapter.name LIKE N'%" . $search . "%'
    LIMIT $start, $pageSize";
} else {
    $getTableDataSql = "SELECT * FROM tbl_chapter LIMIT $start, $pageSize";
}
...
<div class="text-left flex justify-between">
    <button type="button" class="btn btn-primary mb-2 mt-3" data-bs-toggle="modal" data-bs-target="#addChapter">
...
<div class="container p-0">
    <table class="w-100">
        <legend class="text-center"><b>Quản lý các chương</b></legend>

        <thead class="table-head w-100">
            <tr class="table-heading">
                <th class="noWrap">STT</th>
                <th class="noWrap">Mã chương</th>
                <th class="noWrap">Tên chương</th>
                <th class="noWrap">Mô tả</th>
                <th class="noWrap">Số câu hỏi</th>
        ...
        <tbody class="table-body">
            ...
            ?>
                <tr>
                    <td>
                        <?php echo $displaychapter + ($pageIndex - 1) * $pageSize; ?>
                    </td>
                    <td class="tensanpham">
                        <?php echo $row['code'] ?>
                    </td>
                    ...
                    <td>
                        ...
                        <a class='btn btn-primary mb-2 mt-3 con-tooltip top text-white productSizeButton color-white' href="?workingPage=question&chapterId=<?php echo $row['id']; ?>">
            ...
            <ul class="m-0 pagination justify-content-end py-2 col">
                <li class="page-item">
                    ...
                    if ($pageIndex > 1 && $total_page > 1) {
                        echo '<a class="page-link text-reset text-black" aria-label="Previous" href="?workingPage=chapter&limit=' . ($pageSize) . '&page=' . ($pageIndex - 1) . '">
                        Previous
                        </a>';
                    }
                    ?>
                ...
                for ($i = 1; $i <= $total_page; $i++) {
                    if ($i == $pageIndex) {
                        ...
                        <span name="page" class="page-link text-reset text-white bg-dark" href="?workingPage=chapter&limit=' . ($pageSize) . '&page=' . ($i) . '"> ' . ($i) . ' </span>
                        </li>';
                    } else {
                        // Hiển thị trang đầu tiên
                        if ($i == 1) {
                            echo '<li class="page-item light">
                            <a name="page" class="page-link text-reset text-black" href="?workingPage=chapter&limit=' . ($pageSize) . '&page=' . ($i) . '"> ' . ($i) . ' </a>
                            </li>';
                        }
                ...
                if ($pageIndex < $total_page && $total_page > 1) {
                    echo '<li class="page-item light">
                    <a name="page" class="page-link text-reset text-black" aria-label="Next" href="?workingPage=chapter&limit=' . ($pageSize) . '&page=' . ($pageIndex + 1) . '">
                    Next
                    </a>
                    </li>';
                }
...
$tableData = mysqli_query($connect, $getTableDataSql);
while ($row = mysqli_fetch_array($tableData)) {
    include "./pages/Chapter/ChapterConfirmDelete.php";
}
?>
...
$tableData = mysqli_query($connect, $getTableDataSql);
while ($row = mysqli_fetch_array($tableData)) {
    include "./pages/Chapter/EditChapterPopup.php";
}
?>
